...
- 商人（Trader）可用重複卡換卡包。**決定不做**（2026-08-14）：商人跟部落交換在搶同一批
  重複卡，要做得有意義就得算「這張該餵商人還是給隊友」，那是本站刻意排除的全域最佳化。
- 雙方必須同時在線
...
**60 張已全數確認**（2026-08-14）：中文由使用者對照遊戲畫面校正，
英文經官方 wiki 與解鎖等級交叉驗證。**配對邏輯完全不依賴名字**，
所以名字錯不會造成配對錯誤，只是顯示不好看。

驗證過程發現三條結構規律，記在 `cards.json` 的 `_order_rules`，
遊戲更新後可以拿來自我檢查：

1. **相簿順序＝兵營解鎖順序**（聖水／闇黑／建築大師基地）。
   八個獨立驗證點全部命中：Thrower=兵營 18、Meteor Golem=19、Ruin Witch=闇黑兵營 13、
   Sneaky Archer=建築兵營 2、Bomber=5、Night Witch=8、Power P.E.K.K.A=10、Hog Glider=11。
2. **建築大師基地只有 11 張是正確的** —— Electrofire Wizard 不是建築兵營兵種，不是漏掉。
3. **超級部隊照本體兵種的順序排**，先 11 個聖水系再 6 個闇黑系（11+6=17），
   **不是**照大本營等級排。

「飛龍寶寶」家鄉與夜世界同名，卡表刻意把夜世界那張寫成「飛龍寶寶（夜）」，
方便在部落聊天室講清楚是哪一張。
...
- 未登入狀態下用**任何**已綁定的村莊登入，都會進到它原本所屬的帳號 ——
  權杖驗證本身就是所有權證明，不該因為換了小號就登不進來
- 已登入狀態下加綁一個屬於別人帳號的村莊：
  - 來源帳號**只有那一個村莊** → **直接搬過來**，來源帳號刪除（連同其 session）。
    收藏以 tag 為鍵，搬家不掉資料。
    這是為了救「成員先各自單獨登入兩隻帳號才發現有加綁功能」的實際情況，
    否則兩邊永遠合不起來。
  - 來源帳號**還有其他村莊** → 409，訊息要指出是被哪些村莊卡住、以及該怎麼做
- **真正的防線是 `verifytoken`，不是 409**。能走到合併那一步，代表他已經通過
  該村莊的遊戲內權杖驗證。不要把 409 當成安全機制。
...
**交換是 1:1 的**：從「送出候選」挑一張，換「收到候選」的一張。
候選清單有 3 個**不代表能換 3 次**。

```
互利   trades = min(|fills_them|, |fills_me|)
       每張卡補一次空缺就夠了，所以各用一次，張數不是限制

我受益 trades = min(|fills_me|, 我在該系列的多餘「張數」)
我幫人 trades = min(|fills_them|, 對方在該系列的多餘「張數」)
       單向時送出的是對方已有的卡，每換一次就實際消耗一張，
       所以上限看張數不看種類數
```

`gain` / `help` 一律用 `trades` 加總，**不可用候選清單長度加總** ——
那樣會在只能換 4 次的配對上印出「可補你 7 張」。

⚠️ **單向的那一側，候選清單要按張數展開**（同一張有 3 張就出現 2 次），
否則 `trades` 會大於清單長度，介面配對到第二列就顯示空白格（實際發生過）。
`tests/test_trade_limits.py` 有一個跑 300 組隨機資料的不變量測試守這條。

介面必須配成明確的一對一，不可以排成兩排讓人自己猜 ——
會被讀成「這三張一起送出換那一張」而白白浪費卡。
