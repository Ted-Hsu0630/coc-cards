...
# 未擁有的卡是**真灰階**：實測 288 個完整格子裡，111 個未擁有的飽和度
# 連最大值都精確為 0 —— 連 LINE 壓縮過的 JPEG 也一樣，因為純灰區域
# 沒有色度可以被壓縮弄髒。所以門檻多低都安全，真正要小心的是另一邊。
#
# 用第 99 百分位，不是第 95。加農炮戰車的美術是灰色金屬砲管，只有背景
# 和底部繩索有顏色，實測 S95 只有 39 —— 差一分就被判成沒擁有，
# 而那是會靜默寫錯資料的方向。改用 S99 之後間隙從 39 拉開到 111：
#
#     統計量   已擁有最低   未擁有最高
#     S95        39          0        ← 門檻 40，加農炮戰車卡在邊上
#     S99       111          0
#
# 不用最大值：那只要一個雜訊像素就會翻盤。S99 要求 1% 的像素有顏色。
OWNED_SAT = 40
...
def cell_owned(hsv, box, clip="", h_full=None):
    """未擁有的卡是真灰階，飽和度精確為 0。回傳 (是否擁有或 None, S99)。

    不可以用飽和度**中位數** —— 熔岩獵犬（灰岩）中位數 49、
    冰霸王（白冰）中位數 13，兩張都確實擁有。也不可以用 S95：
    加農炮戰車（灰色金屬砲管）S95 只有 39，門檻 40 就差一分判錯。
    理由與實測數字見 OWNED_SAT。

    被切到的格子，取樣範圍必須照**完整卡片高度**推算，不能照可視高度：
    實測一張下緣被切、只剩 50px 的灰階卡，照可視高度取樣會取到卡片上方
    的藍色邊框，飽和度衝高而誤判成「已擁有」。可視部分太少時回 None，
    寧可交給人工也不要猜。
    """
    x, y, w, h = box
    hf = h_full or h
    ...
    if roi.size == 0:
        return None, 0.0
    sat = float(np.percentile(roi[:, :, 1], 99))
    return sat > OWNED_SAT, sat
