...
def test_白色美術不會被誤判成徽章():
    """天使的白髮從上方垂進徽章區，長出 10x35 與 20x33 兩個元件，
    兩者剛好互相對齊而騙過了對齊分群。她其實沒有徽章（就是 1 張）。

    這種誤判不會寫錯資料（數字讀不出來會標成認不出），但使用者會看到
    一格莫名其妙需要人工填 —— 而那格明明清清楚楚。
    """
    cell = _by_id(_resolved("IMG_4978"))["elixir-08"]
    assert not cell.has_badge, "白髮被當成徽章字形了"
    assert cell.count == 1
